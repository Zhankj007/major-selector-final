window.initializePlansTab = function() {
    ...
    const PLAN_ID_KEY = 'plan_unique_id';
    ...
    async function initialize() {
        ...
        try {
            ...
            allPlansData.forEach((plan, index) => { plan[PLAN_ID_KEY] = `${plan['院校代码']}-${plan['专业代码']}-${plan['科类']}-${plan['批次']}-${index}`; });
            buildFilterUI();
    ...
    function buildFilterUI() {
        const filters = { '科类': { data: new Set() }, '城市': { type: 'complex_city' }, '选科': { type: 'complex_subject' }, '院校水平': { type: 'custom_level' }, '办学性质': { data: new Set() }, '本专科': { data: new Set() },};
        allPlansData.forEach(p => { if(p['科类']) filters['科类'].data.add(p['科类']); if(p['办学性质']) filters['办学性质'].data.add(p['办学性质']); if(p['本专科']) filters['本专科'].data.add(p['本专科']); });
        let filterHtml = '';
        Object.keys(filters).forEach(key => { filterHtml += `<details class="filter-group"><summary>${key}</summary><div class="filter-options" id="filter-opts-${key}"></div></details>`; });
        filterContainer.innerHTML = filterHtml;
        ['科类', '办学性质', '本专科'].forEach(key => {
            const optsContainer = container.querySelector(`#filter-opts-${key}`);
            const dataSet = filters[key].data;
            optsContainer.innerHTML = Array.from(dataSet).sort().map(val => `<label><input type="checkbox" data-filter-key="${key}" value="${val}">${val}</label>`).join('');
        });
        const cityTiers = new Set(allPlansData.map(p=>p['城市评级']).filter(Boolean));
        let cityHtml = '<div><strong>城市评级:</strong><div class="filter-options-subgroup">' + Array.from(cityTiers).sort().map(val => `<label><input type="checkbox" data-filter-key="城市评级" value="${val}">${val}</label>`).join('') + '</div></div>';
        container.querySelector('#filter-opts-城市').innerHTML = cityHtml;
        container.querySelector('#filter-opts-选科').innerHTML = `<p>待开发</p>`;
        const levelOptions = ['985','211','双一流大学','基础学科拔尖','保研资格','(省重点建设高校)','(省市共建重点高校)','中外合作办学'];
        container.querySelector('#filter-opts-院校水平').innerHTML = levelOptions.map(val => `<label><input type="checkbox" data-filter-key="院校水平" value="${val}">${val}</label>`).join('');
    }
    ...
    function runQuery() {
        ...
        container.querySelectorAll('#plans-filters-container input:checked').forEach(cb => {
            ...
            if (key) {
                if (params.has(key)) params.set(key, params.get(key) + ',' + cb.value);
                else params.append(key, cb.value);
            }
        });
        treeContainer.innerHTML = `<p>正在根据您的条件查询，请稍候...</p>`;
        fetch(`/api/getPlans?${params.toString()}`)
            .then(res => { if (!res.ok) return res.json().then(err => { throw new Error(err.error || `查询API返回错误: ${res.status}`) }); return res.json(); })
            .then(data => {
                currentRenderedData = data;
                treeContainer.innerHTML = `查询到 ${data.length} 条结果。下一步将实现渲染。`;
            })
            .catch(err => { treeContainer.innerHTML = `<p style="color:red;">查询失败: ${err.message}</p>`; });
    }
    ...
    const observer = new MutationObserver(() => { if(container.classList.contains('active')) updatePasteButtonState(); });
