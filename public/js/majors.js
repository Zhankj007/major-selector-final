window.initializeMajorsTab = function() {
    ...
    function showDetails(targetLi) {
        ...
        // 动态获取所有年份的计划数与录取情况并降序排列（如 2026年、2025年）
        const dynamicPlanKeys = Object.keys(d)
            .filter(key => /^\d{4}年浙江计划数与录取情况$/.test(key))
            .sort((a, b) => b.localeCompare(a));

        const displayOrder = (currentCatalogType === 'bachelor') 
            ? ['门类', '专业类', '学位', '学制', ...dynamicPlanKeys, '设立年份', '指引必选科目', '体检限制', '培养目标', '主要课程', '就业方向或职业面向', '专业知识库链接', '开设院校链接']
            : ['专业大类', '专业类', ...dynamicPlanKeys, '接续高职本科', '接续普通本科', '职业证书', '职业面向', '培养目标', '专业能力', '基础课程', '核心课程', '实习实训'];
