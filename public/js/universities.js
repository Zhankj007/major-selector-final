window.initializeUniversitiesTab = function() {
    ...
    async function fetchData() {
        try {
            ...
            if (!allUniversities || !allUniversities.length) throw new Error("获取的高校数据为空。");
    ...
    function generateFilterOptions() {
        ...
        allUniversities.forEach(uni => { if (!uni) return; (uni['院校水平'] || '').split('/').forEach(level => level.trim() && filters['院校水平'].add(level.trim())); Object.keys(filters).forEach(key => { if (key !== '院校水平' && uni[key]) filters[key].add(uni[key]); }); });
        Object.entries(filters).forEach(([key, valueSet]) => {
            const uiContainer = filterUIs[key]; if (!uiContainer) return;
            ...
            filterGroup.addEventListener('change', () => { const hasSelection = filterGroup.querySelector('input:checked'); filterGroup.querySelector('summary').classList.toggle('filter-active', !!hasSelection); runQuery(); });
        });
    }
    ...
    function runQuery() {
        ...
        const activeFilters = {};
        Object.keys(filterUIs).forEach(key => { const checked = Array.from(filterUIs[key].querySelectorAll('input:checked')).map(cb => cb.value); if (checked.length) activeFilters[key] = new Set(checked); });
        const sortedList = [...allUniversities].sort((a, b) => { const levelA = a['办学层次'] === '本科' ? 1 : 2; const levelB = b['办学层次'] === '本科' ? 1 : 2; if (levelA !== levelB) return levelA - levelB; const codeA = a[UNI_CODE_KEY] || '999999'; const codeB = b[UNI_CODE_KEY] || '999999'; return String(codeA).localeCompare(String(codeB)); });
        let filteredList = sortedList.filter(uni => {
            ...
            for (const [key, valueSet] of Object.entries(activeFilters)) {
                if (key === '院校水平') { const uniLevels = new Set((uni[key] || '').split('/').map(s => s.trim())); if (![...valueSet].some(v => uniLevels.has(v))) return false; }
    ...
    function renderUniversityTree(list) {
        let hierarchy; if (groupBy === 'region') hierarchy = buildHierarchy(list, '省份', '城市'); else hierarchy = buildHierarchy(list, '主管部门');
        ...
        if (!list.length) html += '<li>没有找到匹配的院校。</li>';
            ...
            for(const l1Key in hierarchy) {
                ...
                if(groupBy === 'region') { for(const l2Key in hierarchy[l1Key]) { html += `<li class="level-2-li"><input type="checkbox"> <span class="caret tree-label">${l2Key}</span><ul class="nested">`; hierarchy[l1Key][l2Key].forEach(uni => html += renderUniLi(uni)); html += `</ul></li>`; } }
                else { hierarchy[l1Key].forEach(uni => html += renderUniLi(uni, 'level-2-li')); }
                html += `</ul></li>`;
            }
        }
        html += '</ul>'; treeContainer.innerHTML = html; syncUniCheckboxesWithState(); attachUniEventListeners();
