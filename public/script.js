...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function initializeUniversitiesTab() {
        ...
        function showUniDetails(li) {
            ...
            const shortFields = ['办学性质', '办学层次', '院校类型', '城市评级', '985', '211', '双一流'];
            const longFields = [UNI_CODE_KEY, UNI_NAME_KEY, '省份', '城市', '主管部门', '院校水平', '官网', '招生章程'];
            let html = '';
            ...
            const longFieldsHtml = longFields.map(key => {
                ...
                return `<p><strong>${key}:</strong> <span>${value}</span></p>`;
            }).join('');
    ...
    function initializeMajorsTab() {
        ...
        function renderTreeHTML(hierarchy, type, autoExpand = false) {
            const majorNameKey = '专业名'; const majorCodeKey = '专业码'; let html = '<ul>';
            if (Object.keys(hierarchy).length === 0) { html += '<li>没有找到匹配的专业。</li>'; }
            else { for (const level1Key in hierarchy) { html += `<li class="level-1-li"><input type="checkbox"> <span class="caret ${autoExpand ? 'caret-down' : ''} tree-label">${level1Key}</span><ul class="nested ${autoExpand ? 'active' : ''}">`; for (const level2Key in hierarchy[level1Key]) { const majors = hierarchy[level1Key][level2Key]; html += `<li class="level-2-li"><input type="checkbox"> <span class="caret ${autoExpand ? 'caret-down' : ''} tree-label">${level2Key}</span><ul class="nested ${autoExpand ? 'active' : ''}">`; majors.sort((a, b) => (String(a[majorCodeKey] || '').localeCompare(String(b[majorCodeKey] || '')))); majors.forEach(major => { const detailsBase64 = btoa(encodeURIComponent(JSON.stringify(major))); const majorName = major[majorNameKey] || '未知专业'; const majorCode = major[majorCodeKey] || ''; html += `<li class="level-3-li" data-details="${detailsBase64}"><input type="checkbox" value="${majorName}"><span class="major-label">${majorName} (${majorCode})</span></li>`; }); html += '</ul></li>'; } html += '</ul></li>'; } }
            html += '</ul>'; return html;
        }
