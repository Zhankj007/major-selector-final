...
网站是标准三层架构的 Web 应用，页面采取标签页模式，一个标签页代表一种功能查询，目前已搭建高校库、专业目录、2025浙江高考招生计划、个人测评、后台管理等5个标签页，每个标签页一个前端文件，互相之间不干扰。<br><br>
...
### 模块功能
:blue_heart:**高校库** <br>直接在前端服务器上 _data 目录下建立数据源 universities.csv 文件，将全国截止2025年7月的高校名单信息纳入。页面左侧，按省份、主管部门等两种架构进行树状结构查询，同时建立了院校水平、院校类型、城市评级、办学性质、办学层次等五个维度的筛选器，并提供院校名称的关键字查询。页面右侧，上方提供院校详情的查看，只要在左侧院校名上悬停或点击，右侧自动出现该院校的详情信息，同时融入2027年各在浙招生高校的专业选考科目要求，双击院校名称即出现选科要求；下方提供意向院校的收藏，只要将左侧院校名前面的方框勾选，自动将该院校纳入意向院校清单，并提供复制和清空按钮。<br>
...
:blue_heart:**个人测评** <br>在Supabase数据库上搭建了专业规则表（major_rules）,分别匹配了霍兰德职业兴趣测评、MBTI性格倾向测评、个人能力自评的代码，使测评代码与本科专业相对应；另外，根据三类测评内容，分别设计了不同数量的题库（questions）和答案选项分值（choices）。进入个人测评页面后，点击开始测评，系统自动从题库中抽取42道holland，28道MBTI，30道个人能力，共计100题。测评结束，系统根据用户答题情况，自动生成霍兰德、MBTI及能力代码，并根据代码在专业规则表里匹配相关专业，并按匹配值从高到低生成10个推荐专业供用户参考，用户点击生成的推荐专业，还能弹出专业详情信息供查看。<br>
